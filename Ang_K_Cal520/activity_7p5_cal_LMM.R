...
boxplot(activity ~ genotype, data_table)
ggplot(aes(genotype, activity), data = data_table) +  geom_point()  

# Find the correct underlying probability distribution
hist(data_table$activity, breaks=50)
ggplot(aes(genotype, activity), data = data_table) + geom_beeswarm(dodge.width = 1)

mean(data_table[data_table$genotype == 'wt', 'activity'])
mean(data_table[data_table$genotype == 'KO', 'activity'])

m1.norm <- glmmTMB(activity ~ genotype + (1|recording), 
                    data = data_table)
m1.lnorm <- glmmTMB(activity ~ genotype + (1|recording), 
                    family = gaussian(link="log"), data = data_table)
m1.gamma <- glmmTMB(activity ~ genotype + (1|recording), 
                    family = Gamma(link="log"), data = data_table)
#m1.tweedie <- glmmTMB(activity ~ genotype + (1|recording), 
...
# reduced base model
model.base <- glmmTMB(activity ~ 1 + (1|animal),family = Gamma(link="log"),
                       data = data_table)

# full model
model <- glmmTMB(activity ~ genotype + (1|animal),family = Gamma(link="log"),
                  data = data_table)
...
write.csv(anova_result,paste("anova_result_activity_7p5_cal_",today,".csv",sep=""))
