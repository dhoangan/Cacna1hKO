...
data = read.csv("ttap2_active_cells_per_slice.csv", sep=",")
...
# reduced base model
model_KO.base <- glmmTMB(n_percentage_active_cells ~ 1 + (1|recording),
                         data = data_subset_KO)

# full model
model_KO <- glmmTMB(n_percentage_active_cells ~ genotype_2 + (1|recording),
                    data = data_subset_KO)
summary(model_KO)
